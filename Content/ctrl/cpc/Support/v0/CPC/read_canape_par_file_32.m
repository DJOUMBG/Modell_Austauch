% data = read_canape_par_file_32(name) lädt Parameter aus der Datei name (mit
% CANape CDM-Studio (Version 3.2) gespeichert) und speichert diese in der
% Variable data ab. Dabei werden die SI-Werte und nicht die RWV-Werte
% gespeichert!
%
% Autor: Stefan Goldschmidt, Daimler AG, 24.06.2008

function [data, datatype] = read_canape_par_file_32(name, flag)
...
while ischar(l)
    ...
    while (s >= 0 && s <= 5) % ... solange kein Fehler vorliegt (s < 0) oder das Ende erreicht wurde (s > 5)
        ...
        if (p > length(l) && s == 3); s = 6; p_info(2) = p-2; p_wert = []; % Wert fehlt
        elseif (p > length(l) && s ~= 6); s = -1; 
        end % Fehler bei Überlauf
    ...
    if (s == 6) % ... es wurde eine Zeile erfolgreich eingelesen
        ...
        if ~isempty(p_wert); wert = l(p_wert(1):p_wert(2)); else wert = ''; end
