...
def posthook(configuration_xml_file, main_sil_name, dInitOrderModels):
    ...
    for key, value in sorted(dInitOrderModels.items()):
        if "--"  in value:
            ...
            if plt_human_match:
                ...
                print("WARNING: could not add the sfu  " + value + " to posthook list.\n")
        else:
    ...
    for info in posthook_info_list:
        ...
        for script_name in info[2]:
            ...
            print("    executing posthook <" + script_path + ">")
            #              1           2             3             4        5         6
...
def return_module_path(module_xml_folder, type):
    ...
    try:
        ...
        print("ERROR in posthook, can't read module xml as it is trying to reference external resource...")
        return ""
...
def code_wrapper(pathPosthook, pathPosthookPy, pathMainSil, listPathSFUs, dictModule, etreeCfg):
    ...
    try:
        ...
        print("ERROR in " + pathPosthookPy + ", " + str(e))
    # restore original wd
    os.chdir(current_wd)
    ######################
